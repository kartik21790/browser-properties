...
function getBrowserProperties($output="array")
{
	...
	else
	{
		...
		//referred http://detectmobilebrowsers.com/ for exhaustive list of mobile keywords in user-agent
		...
		//referred http://php.net/manual/en/function.get-browser.php#101125 for below regex logic
		...
		elseif(preg_match("/Trident/i",$u_agent)) // special case added for IE 11
		{
			$bname = 'Internet Explorer';
			$ub = "rv";
		}
		elseif(preg_match("/Edge/i",$u_agent)) // special case added for Microsoft Edge
		{
		...
		//special case added for IE 11 which uses format rv : 11.0 instead of Browser/Version
		if($ub=="rv"){
			$pattern = '#(?<browser>' . $ub .	')[: ]+(?<version>[0-9.|a-zA-Z.]*)#';
		}
